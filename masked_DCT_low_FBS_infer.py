...
# set the ratio of the FBS trajectory to the whole sampling trajectory
...
# img_path = 'test1.jpg'
# mask_path = 'test1_mask.jpg'

img_path = 'test2.jpg'
mask_path = 'test2_mask.jpg'
...
mask = np.array(Image.open(mask_path).resize((W, H)), dtype=np.float32)
mask = mask / 255.  # 0 ~ 1
mask_tensor = torch.from_numpy(mask).permute(2, 0, 1)[None, ...].cuda()  # 1, 3, h, w
mask_tensor = torch.mean(mask_tensor, dim=1, keepdim=True)  # 1, 1, h, w
latent_mask_tensor = torch.nn.functional.interpolate(input=mask_tensor, size=(H // 8, W // 8))  # 1, 1, h/8, w/8
latent_mask_tensor = torch.where(latent_mask_tensor >= 0.5,
                                 torch.ones_like(latent_mask_tensor), torch.zeros_like(latent_mask_tensor))
...
gen_z0 = sampler.decode_with_masked_low_FBS(ref_guidance=intermediates, mask=latent_mask_tensor, cond=cond,
                                            t_dec=decode_steps,
...
# Image.fromarray(gen_x0).save('test1_low-FBS_res_with_mask.jpg')
Image.fromarray(gen_x0).save('test2_low-FBS_res_with_mask.jpg')