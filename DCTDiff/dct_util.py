...
def dct_coefficients_swap(x_dec_dct, ref_dct, z0_dct, percentile):
    _, c, h, w = x_dec_dct.shape

    top_k = int(h * w * percentile / 100)

    assert top_k <= h*w, 'invalid top_k value'

    z0_dct = torch.reshape(z0_dct, shape=(c, h * w))  # c, hw
    x_dec_dct = torch.reshape(x_dec_dct, shape=(c, h * w))    # c, hw
    ref_dct = torch.reshape(ref_dct, shape=(c, h * w))        # c, hw
    _, ref_positive_indices = torch.sort(z0_dct, descending=True, dim=-1)   # c, hw
    _, ref_negative_indices = torch.sort(z0_dct * (-1), descending=True, dim=-1)  # c, hw

    ref_positive_indices = ref_positive_indices[:, :top_k]
    ref_negative_indices = ref_negative_indices[:, :top_k]

    x_dec_dct.scatter_(src=torch.gather(input=ref_dct, dim=-1, index=ref_positive_indices),
                       index=ref_positive_indices, dim=-1)

    x_dec_dct.scatter_(src=torch.gather(input=ref_dct, dim=-1, index=ref_negative_indices),
                       index=ref_negative_indices, dim=-1)

    x_dec_dct = torch.reshape(x_dec_dct, shape=(-1, c, h, w))  # n, c, h, w
