...
dct_calibration_ratio = 0.45

# the end step of the calibration phase
end_step = 1000 - 1000 * dct_calibration_ratio
...
target_prompt = 'painting of a robot'
...
img = np.array(Image.open(img_path).resize((H, W)), dtype=np.float32)
...
gen_z0 = sampler.decode_with_mid_FBS(ref_guidance=intermediates, cond=cond, t_dec=decode_steps,
                                     unconditional_guidance_scale=unconditional_guidance_scale,
                                     unconditional_conditioning=un_cond, mp_percentiles=(7, 50), end_step=end_step)
...
Image.fromarray(gen_x0).save('test2_mid-FBS_res.jpg')
